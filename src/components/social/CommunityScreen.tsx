...
function StatPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center border border-border rounded-lg px-4 py-3 flex-1">
      <span className="text-[22px] font-bold text-text tabular-nums">{value}</span>
      <span className="text-[10px] font-medium uppercase tracking-widest text-muted mt-0.5">
        {label}
      </span>
    </div>
  );
    ...
    <div className="shrink-0 w-full px-4 pt-6 pb-24">
      ...
      <div className="flex gap-2 mb-6">
        ...
        <StatPill
          ...
          value={data ? String(data.activeThisWeek) : "—"}
        />
        <StatPill
          label={`Total ${unit} lifted`}
          value={data ? formatVolume(data.totalVolumeKg) : "—"}
        />
      </div>
